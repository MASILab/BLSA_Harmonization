...
# find current path (assumed to be run in harmonization folder)
...
for b in tqdm([x for x in Old_dir.glob('BLSA_*') if sub_pattern.match(x.name)]):
    ...
    for fmri in scans_path.glob('*r*/NIFTI'):
        if not fmri.exists(): 
            ...
            try:
                # extract run number
                fn = [x for x in fmri.glob('*_FN_r*.ni*')]
                if fn:
                    ...
                    SymlinkPath = str(New_dir) + "/" + subj_id + "/" + sess_id + "/func/"+  subj_id + '_' + sess_id +'_task-fn_run-' + run_id[0] + '_bold' + ".nii.gz"
                    ...
                    if fp:
                        ...
                        if len(run_id) > 1:
                            ...
                            run_id = re.split(".nii.gz",run_id[1])
                        # create symlink path and name
                        SymlinkPath = str(New_dir) + "/" + subj_id + "/" + sess_id + "/func/"+  subj_id + '_' + sess_id +'_task-fp_run-' + run_id[0] + '_bold' + ".nii.gz"
                        # get bash line to create symlink and print it out
